...
for i in range(29):
    ...
    #企業名をリストに入れる
    corporates = []
    for corporate in driver.find_elements_by_class_name('corporateName'):
        corporates.append(corporate.text)
    #説明をリストに入れる
    service_catches = []
    ...
    #詳細情報をリストに入れる
    infos = []
    for info in driver.find_elements_by_xpath('//div[@class="btnsWrap"]/a'):
        infos.append(info.get_attribute('href'))
